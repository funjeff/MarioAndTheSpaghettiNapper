...
s32 act_riding_shell_ground(struct MarioState *m) {
    ...
    if ((m->controller->buttonPressed & L_TRIG) && !m->lHeld) {
...
//	s16 startYaw = m->faceAngle[1];
	...
	if (m->input & INPUT_A_PRESSED) {
	    return set_mario_action(m, ACT_SWINGING_ROPE_JUMP, 0);
	}
    ...
    switch (perform_ground_step(m)) {
        case GROUND_STEP_LEFT_GROUND:
            set_mario_action(m, ACT_SWINGING_ROPE_FALL, 0);
            break;
    }
//	tilt_body_ground_shell(m, startYaw);
...
s32 mario_execute_moving_action(struct MarioState *m) {
    ...
    switch (m->action) {
        ...
        case ACT_SWINGING_ROPE:            cancel = act_swinging_rope(m);            break;
    }
    /* clang-format on */
